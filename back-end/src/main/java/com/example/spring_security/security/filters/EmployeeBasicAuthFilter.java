...
import com.example.spring_security.security.authentications.EmployeeAuthentication;
...
public class EmployeeBasicAuthFilter extends OncePerRequestFilter {
    @Autowired
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        try {
            ...
            EmployeeAuthentication authRequest = new EmployeeAuthentication(token.getName(), token.getCredentials());
            EmployeeAuthentication authResult = (EmployeeAuthentication) authenticationManager.authenticate(authRequest);
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
        return !request.getServletPath().equals("/login-for-employee");
    }
}
